...
class PlanningGraph():
        ...
        for action in self.all_actions:
            ...
            for pos_precond in action.precond_pos:
                ...
                for pg_node_s in s_level:
                    if PgNode_s(pos_precond, True).literal == pg_node_s.literal:
                        precondition_satisfied = True
                        precondition_parents.append(pg_node_s)
            ...
            for neg_precond in action.precond_neg:
                precondition_satisfied = False
                for pg_node_s in s_level:
                    if PgNode_s(neg_precond, False).literal == pg_node_s.literal:
                        precondition_satisfied = True
                        precondition_parents.append(pg_node_s)
                if not precondition_satisfied:
                    all_preconditions_satisfied = False
            ...
            if all_preconditions_satisfied:
                ...
                for pg_node_s in precondition_parents:
                    new_pgnode_a.parents.add(pg_node_s)
                a_level.add(new_pgnode_a)
    ...
    def add_literal_level(self, level):
        ...
        for a_node in a_level:
            for effect_node in a_node.effect_s_nodes():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for effect in a1_effect_add:
            if effect in a2_effect_rem:
                return True

        for effect in a1_effect_rem:
            if effect in a2_effect_add:
                return True

        for effect in a2_effect_add:
            if effect in a1_effect_rem:
                return True

        for effect in a2_effect_rem:
            if effect in a1_effect_add:
                return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for src_effect in a1_effect_add:
            if src_effect in a2_precond_neg:
                return True

        for src_effect in a1_effect_rem:
            if src_effect in a2_precond_pos:
                return True

        for src_effect in a2_effect_add:
            if src_effect in a1_precond_neg:
                return True

        for src_effect in a2_effect_rem:
            if src_effect in a1_precond_pos:
                return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1_parent_nodes = node_a1.parents
        a2_parent_nodes = node_a2.parents

        for a1_parent_node in a1_parent_nodes:
            for a2_parent_node in a2_parent_nodes:
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        s1_parent_actions_nodes = node_s1.parents
        s2_parent_actions_nodes = node_s2.parents

        for s1_action_node in s1_parent_actions_nodes:
            for s2_action_node in s2_parent_actions_nodes:

                if not s1_action_node.is_mutex(s2_action_node):
                    return False
